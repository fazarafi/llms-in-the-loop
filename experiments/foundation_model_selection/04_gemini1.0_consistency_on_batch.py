...
RESULTS_PATH = os.path.join(PATH_TO_SRC, 'data/foundation_model_selection/gemini-1.0_batch_size_2')
...
def repeat_annotation(n_repeat=10, **ask_gemini_kwargs):
    ...
    for i in tqdm(range(n_repeat)):
        ...
        if match:
            ...
            try:
                outputs = json.loads(json.dumps(ast.literal_eval(model_response)))['output']
...
for index in range(len(sampled_subset)//2):
    ...
    ask_gemini_params['tokens'] = tokens

    # Query the model
    new_labels_gemini = repeat_annotation(n_repeat=REPEAT_ANNOTATION,
                                            **ask_gemini_params)
    
    # Save annotations
    gemini_annotations[f'record_{i}'] = {}
    gemini_annotations[f'record_{i}']['pred'] = new_labels_gemini['record_0']
    gemini_annotations[f'record_{i}']['true'] = ground_truth_labels_record_0
    gemini_annotations[f'record_{i}']['tokens'] = record_0['tokens']
    
    gemini_annotations[f'record_{i+1}'] = {}
    gemini_annotations[f'record_{i+1}']['pred'] = new_labels_gemini['record_1']
    gemini_annotations[f'record_{i+1}']['true'] = ground_truth_labels_record_1
    gemini_annotations[f'record_{i+1}']['tokens'] = record_1['tokens']

    # Calculate consistency score
    consistency_record_0 = calculate_consistency_score(new_labels_gemini['record_0'],
                                                ground_truth_labels_record_0)
    gemini_annotations[f'record_{i}']['consistency'] = consistency_record_0
    consistency_scores.append(consistency_record_0)

    consistency_record_1 = calculate_consistency_score(new_labels_gemini['record_1'],
                                                ground_truth_labels_record_1)
    gemini_annotations[f'record_{i+1}']['consistency'] = consistency_record_1
